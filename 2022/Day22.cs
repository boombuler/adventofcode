...
{
    private static readonly Point[] Directions = [Point.Right, Point.Down, Point.Left, Point.Up];
    delegate (int Direction, Point Position) GetPositionDelegate(int direction, Point Pt);
    class CubeFace(Point mapIndex)
    {
        public (CubeFace Face, int Direction)[] Sides { get; } = new (CubeFace Face, int Rotate)[Directions.Length];
        public Point MapIndex { get; } = mapIndex;
    }
    ...
    private static long GetFinalPositionOnMap(string input)
        => GetFinalPosition(input, map =>
        {
            ...
            var coerceBounds = new Func<Point, Point>[]
            {
    ...
    private static Point GetFaceIndex(Point pt, int cubeSize) => pt / cubeSize;

    private static Dictionary<Point, CubeFace> FoldCube(IEnumerable<Point> points, int cubeSize)
    {
    ...
    private static long GetFinalPosition(string input, Func<IEnumerable<Point>, GetPositionDelegate> buildGetPosFunc)
    {
        var (mapInput, (directionInput, _)) = input.Split("\n\n");
        var map = mapInput.Cells(c => c == '#', c => c != ' ');

        var position = new Point(map.Keys.Where(p => p.Y == 0).Min(p => p.X), 0);
        var dir = 0;
