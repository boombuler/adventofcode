...
namespace _2020_22
{
    class Program : ProgramBase
    {
        ...
        private IEnumerable<int> GetWinnerDeck(string input, bool recursive)
        {   
            ...
            var p1 = new LinkedList<int>(ReadLines(input).Skip(1).TakeWhile(l => !string.IsNullOrEmpty(l)).Select(int.Parse));
            var p2 = new LinkedList<int>(ReadLines(input).SkipWhile(l => !string.IsNullOrEmpty(l)).Skip(2).Select(int.Parse).ToList());

            return Play(p1, p2).Deck;
        }

        private long GetWinningScore(string inputFile, bool recursive)
