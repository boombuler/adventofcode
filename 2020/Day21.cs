...
namespace AdventOfCode._2020
{
    ...
    {
        private long CountAllergenFreeIngredients(string input)
        {
            (var Ingredients, var Allergens) = LoadAllergenList(input);
            return Ingredients.Keys.Except(Allergens.Select(a => a.Ingredient)).Sum(i => Ingredients[i]);
        }

        private string GetDangerousList(string input)
        {
        ...
        private (Dictionary<string, int> Ingredients, List<(string Allergen, string Ingredient)> Allergens) LoadAllergenList(string input)
        {
