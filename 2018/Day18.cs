...
namespace AdventOfCode._2018
{
    class Day18 : Solution
    {
        ...
        private State NextState(State cell, IEnumerable<State> neighbours)
        {
            switch(cell)
            {
                case State.Open:
                    return neighbours.Count(n => n == State.Tree) >= 3 ? State.Tree : State.Open;
                case State.Tree:
                    return neighbours.Count(n => n == State.Lumberyard) >= 3 ? State.Lumberyard : State.Tree;
                case State.Lumberyard:
                    return neighbours.Any(l => l == State.Lumberyard) && neighbours.Any(l => l == State.Tree) ? State.Lumberyard : State.Open;
                default:
                    return cell;
            }
        }

        private State[][] Read(string map)
        {
            return map.Lines().Select(l => l.Select(c => c switch
            {
                '|' => State.Tree,
                '.' => State.Open,
                _ => State.Lumberyard
        ...
        private long Simulate(string map, int rounds)
        {
            var cur = Read(map);
            int size = cur.Length;
