...
abstract class Solution<TSolution1, TSolution2> : AsyncSolution<TSolution1, TSolution2>, ISolution
{
    ...
    {
        using (UseOutput(o))
            return Task.FromResult(Convert.ToString(actn()) ?? string.Empty);
    }
...
abstract class AsyncSolution<TSolution1, TSolution2> 
{
    ...
    public virtual int Year => int.Parse(TrailingInt.Match(GetType().Namespace ?? string.Empty).Value); // Override when this throws an exception
    ...
    protected void Assert(bool result, string? name = null)
        => fOutput.Value?.Assertion(name, result, string.Empty);
