...
namespace AdventOfCode._2017
{
    class Day20 : Solution
    {
        private class Particle
        {
            ...
            private static Func<string, Point3D> ParsePosition     = new Regex(@"p=<(?<x>-?\d+),\s?(?<y>-?\d+),\s?(?<z>-?\d+)>", RegexOptions.Compiled).ToFactory<Point3D>();
            private static Func<string, Point3D> ParseVelocity     = new Regex(@"v=<(?<x>-?\d+),\s?(?<y>-?\d+),\s?(?<z>-?\d+)>", RegexOptions.Compiled).ToFactory<Point3D>();
            private static Func<string, Point3D> ParseAcceleration = new Regex(@"a=<(?<x>-?\d+),\s?(?<y>-?\d+),\s?(?<z>-?\d+)>", RegexOptions.Compiled).ToFactory<Point3D>();
